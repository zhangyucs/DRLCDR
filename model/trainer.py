...
from model.DRLCDR import DRLCDR
...
class CrossTrainer(Trainer):
    def __init__(self, opt):
        self.opt = opt
        if self.opt["model"] == "DRLCDR":
            self.model = DRLCDR(opt)
        else :
    ...
    def save_v(self):
        torch.save(self.source_user, self.opt['save_dir']+'/'+self.opt['id']+"/source_user.pt")
        torch.save(self.target_user, self.opt['save_dir']+'/'+self.opt['id']+"/target_user.pt")
        torch.save(self.source_item, self.opt['save_dir']+'/'+self.opt['id']+"/source_item.pt")
        torch.save(self.target_item, self.opt['save_dir']+'/'+self.opt['id']+"/target_item.pt")

    def save_KL(self):
        self.model.save_KL()
        KLlog = open(self.opt['save_dir']+'/'+self.opt['id']+'/KL.result',mode="a")
        print(self.model.save_KL(), file=KLlog)
    ...
    def reconstruct_graph(self, batch, source_UV, source_VU, target_UV, target_VU, source_adj=None, target_adj=None, epoch = 100):
        ...
        if epoch <= self.warmup_epoch:
            ...
            self.source_user, self.source_item, self.target_user, self.target_item = self.model(source_UV,source_VU, target_UV,target_VU, epoch)
        ...
        pos_labels, neg_labels = torch.ones(pos_source_score.size()), torch.zeros(pos_source_score.size())
